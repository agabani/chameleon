...
use crate::hooks::lobby::use_lobby;
...
fn Content(props: &Props) -> HtmlResult {
    ...
    let host = lobby
        .try_get_resources()
        .and_then(Resources::try_get_individual)
        .and_then(|r| r.try_get_relationship("host", "Host"))
        .and_then(|r| r.try_get_resource_identifiers("host"))
        .and_then(|r| r.try_get_individual("host", "Host"))
        .and_then(|r| r.try_get_field(|a| a.id.as_ref(), "id", "Id"))
        .unwrap();
